...
from jeslee_web.ideal.utils import calculate_hash, create_description, amount_in_cents
...
class IdealOrder(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # context_data['hash'] = "TODO"
        valid_util_timestamp = datetime.now() + timedelta(minutes=15)
        valid_util = valid_util_timestamp.strftime('%Y-%m-%dT%H:%M:%S:%fZ')
        context_data['hash'] = calculate_hash(self.object, valid_util=valid_util, order_quantity=1)
        context_data['payment_type'] = settings.IDEAL_PAYMENT_TYPE
        context_data['valid_util'] = valid_util
        context_data['item_description'] = create_description(self.object)
        context_data['item_quantity'] = create_description(self.object)
