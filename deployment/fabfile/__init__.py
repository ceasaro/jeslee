...
provided_by_config = (config.development, config.production)
...
def upgrade():
    ...
    if not prompt('Upgrading {} with {}. Sure? CTRL-C to abort'.format(
                       env.hosts, env.svn_branch), default='y'):
        abort('Abort')
    pack()
    upload()
    backup()
...
def pip_install_requirements():
    ...
    pip.bundle()
    require('pip_bundle')
    puts = put(env.pip_bundle, '/tmp/')
    env.pip_bundle_uploaded = puts[0]
    with virtualenv.context(env.virtualenv_dir):
...
def backup():
    ...
    with TmpDirectory(prefix='caire-backup') as tmp_dir:
        ...
        with cd(backup_dir):
            ...
            sudo('tar -cjf caire.tar.bz2 {}'.format(env.install_dir))
